...
class FrontController extends Controller
{
    ...
    public function homePage() {
        //start
        $banners1 = BannerGroup::query()->with(['galleries.image'])->find(12);
        $banners2 = BannerGroup::query()->with(['galleries.image'])->find(11);
        $banners3 = BannerGroup::query()->with(['galleries.image'])->find(9);
        // bài viết mới nhất
        ...
        $allCateIds = array_keys($catToTop);
        $allPosts = Post::query()->with(['category', 'image'])
        ...
        foreach ($allPosts as $post) {
            $topId = $catToTop[$post->cate_id] ?? null;
            if (!$topId) continue;

            if (!isset($grouped[$topId])) $grouped[$topId] = collect();

            if ($grouped[$topId]->count() < 5) {
                $grouped[$topId]->push($post);
            }
        }
        $parents->each(function ($p) use ($grouped) {
            $p->setRelation('posts', $grouped[$p->id] ?? collect());
        });
        $categories = $parents;

        // bài viết phổ biến (danh mục đặc biệt)
        $popularCate = CategorySpecial::query()->where('id', 15)->first();
        $popularPosts = $popularCate->posts()->with(['image', 'category'])
            ->where('status', 1)->latest()->limit(5)->get();

        // bài viết nổi bật (danh mục đặc biệt)
